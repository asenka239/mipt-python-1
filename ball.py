...
def exit1(event):
	...
	result_table = []
	try:
		with open('res.txt', 'r') as ouf:
			result_table = [x.split(' ')[::-1] for x in ouf.read().split('\n')][:-1]
		result_table = [[int(x[0]), x[1]] for x in result_table]
	except FileNotFoundError:
		with open('res.txt', 'w') as ouf:
			pass
		result_table = list()
	name = input('your name: ')
	result_table.append([count, name])
	result_table = sorted(result_table)
	result_table.reverse()
	with open('res.txt', 'w') as ouf:
		for score in result_table:
			ouf.write(str(score[1]) + ' ' + str(score[0]) + '\n')
		ouf.close()
	for score in result_table[:10]:
